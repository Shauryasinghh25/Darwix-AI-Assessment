...
test_reports_DIR = BASE_DIR / "test_reports" / "Voice" / "recordings"

sys.path.extend([
    str(BASE_DIR / "voice_assistant" / "src"),
    str(BASE_DIR / "knowledge_system" / "src"),
    str(BASE_DIR / "regional_bots" / "src"),
    str(BASE_DIR / "realtime_analytics" / "src"),
])
...
KB_DIR = BASE_DIR / "knowledge_system"
...
class DemoHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if route == "/health":
            ...
            return
        if route in {"/", "/Voice", "/Knowledge", "/Regional", "/Realtime"}:
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if route == "/api/Voice/scenario":
            ...
            return
        if route == "/api/Voice/save-recording":
            saved = self._save_recording(payload)
            self._json(200, saved)
            return
        if route == "/api/Knowledge/build":
    ...
    def _save_recording(self, payload: dict) -> dict:
        ...
        audio_path = test_reports_DIR / f"{name}.webm"
        transcript_path = test_reports_DIR / f"{name}.json"
        if audio_data:
