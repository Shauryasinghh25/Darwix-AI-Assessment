...
import agent
from agent import build_lead_summary, respond
...
OUT = ROOT / "test_reports" / "regression-results.json"
...
def Voice_cases() -> list[dict]:
    ...
    assert_true(long_turn.intent in {"lead_opening", "family_coverage", "fallback"}, "Long Voice input should not crash")
    ...
    for _ in range(100):
        assert_true(respond("When is renewal notice sent?").intent == "grounded_kb_answer", "Rapid Voice request failed")

    original_kb_path = agent.KB_PATH
    try:
        agent.KB_PATH = ROOT / "knowledge_system" / "out" / "__missing_kb__.json"
    ...
    results.extend([
        ...
        {"case": "ST-01 Voice 100 rapid requests", "status": "passed"},
        {"case": "ST-02 Voice long input", "intent": long_turn.intent},
        {"case": "ST-03 Voice emoji input", "intent": emoji_turn.intent},
        {"case": "ST-04 Voice KB unavailable", "intent": unavailable.intent},
    ])
    return results


def Knowledge_cases() -> list[dict]:
    kb = load_kb(ROOT / "knowledge_system" / "out" / "kb.json")
    cases = [
    ...
    assert_true(bool(kb.search("My Aadhaar is 1234 5678 9012", top_k=3)), "TC-14 PII query should route to PII handling")
    ...
    return results + [
        ...
        {"case": "ST-05 Knowledge 100 rapid requests", "status": "passed"},
    ]


def Regional_cases() -> list[dict]:
    cases = [
    ...
    for _ in range(100):
        assert_true(id_analyze("Saya mau cek DP dan tenor")["intent"] == "installment_or_terms", "Rapid Regional request failed")
    results.extend([
        {"case": "ST-06 Regional Hindi/English mix", "intent": "family_coverage"},
        {"case": "ST-07 Regional emoji", "intent": "greeting"},
        {"case": "ST-08 Regional 100 rapid requests", "status": "passed"},
    ])
    return results


def Realtime_cases() -> list[dict]:
    cases = [
    ...
    for _ in range(100):
        assert_true(stream_call(["Customer: I bought another car."])["nudges"], "Rapid Realtime request failed")
    return results + [
        {"case": "TC-07/08 noisy and ambiguous", "nudges": 0},
        {"case": "TC-12 priority order", "signals": multiple_kinds},
        {"case": "ST-09 Realtime 100 rapid requests", "status": "passed"},
    ]


def main() -> None:
    report = {"Voice": Voice_cases(), "Knowledge": Knowledge_cases(), "Regional": Regional_cases(), "Realtime": Realtime_cases()}
